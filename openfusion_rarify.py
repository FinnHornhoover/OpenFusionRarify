...
def generate_patch(
    base_obj: Dict[str, Any], updated_obj: Dict[str, Any]
) -> Dict[str, Any]:
    target_obj = {}
...
class ItemConfig:
    ...
    number_of_crates_till_item_per_mob_and_crate_type: Union[
        None, Dict[int, Dict[str, int]]
    ]
    number_of_crates_till_item_per_crate_type_and_mob: Union[
        None, Dict[str, Dict[int, int]]
    ]
        ...
        if criterion == "number_of_crates_till_item_per_crate_type":
            ...
            for key, value in criteria[criterion].items():
                crate_name_sanitized = (
                    re.sub(r"[^\w\s\d]", "", key.lower()).replace("crate", "").strip()
                )
...
def alter_chances(knowledge_base: KnowledgeBase, item_configs: List[ItemConfig]):
    ...
    itemset_ir_ids = {
        is_id: {
            ir_id for ir_id in itemset["ItemReferenceIDs"] if ir_id in affected_items
        }
    ...
    cdt_ir_ids = {
        cdt["CrateDropTypeID"]: {
            ir_id for cc in cdt["CrateIDs"] for ir_id in crate_ir_ids.get(cc, set())
        }
    ...
    cdt_cdc_ir_ids = {
        (
            cdt_id,
            knowledge_base.drop_maps["CrateDropChances"][mobdrop["CrateDropChanceID"]][
                "CrateDropChanceID"
            ],
        ): ir_id_set
    ...
    def crate_symbols(
        gender_id: int, c_id: int, target_ir_ids: Set[int]
    ) -> Dict[int, Expr]:
        ...
        filtered_target_ir_ids = {
            ir_id for ir_id in itemset["ItemReferenceIDs"] if ir_id in target_ir_ids
        }
        ...
        for rarity_id, rw in zip(range(1, 5), rarity_weights):
            weights_filtered = {
                ir_id: itemset["AlterItemWeightMap"].get(
                    str(ir_id), itemset["DefaultItemWeight"]
                )
                for ir_id in itemset["ItemReferenceIDs"]
                if (
                    (
                        itemset["IgnoreGender"]
                        or itemset["AlterGenderMap"].get(
                            str(ir_id),
                            knowledge_base.item_map[
                                knowledge_base.irid_tuple_map[ir_id]
                            ].get("m_iReqSex", 0),
                        )
                        in [0, gender_id]
                    )
                    and (
                        itemset["IgnoreRarity"]
                        or itemset["AlterRarityMap"].get(
                            str(ir_id),
                            knowledge_base.item_map[
                                knowledge_base.irid_tuple_map[ir_id]
                            ].get("m_iRarity", 0),
            ...
            weights_sym.update(
                {
                    ir_id: (1.0 - target_weight_sum) * weight / non_target_weight_sum
    ...
    def drop_symbols(
        cdt_id: int, cdc_id: int, target_ir_ids: Set[int]
    ) -> List[Dict[int, Expr]]:
        ...
        for gender_id in range(1, 3):
            ...
            for crate_id, crate_w in zip(cdt["CrateIDs"], cdc["CrateTypeDropWeights"]):
                for ir_id, sym_w in crate_symbols(
                    gender_id, crate_id, target_ir_ids
                ).items():
                    mob_agg[ir_id] += (
                        sym_w
                        * crate_w
                        * cdc["DropChance"]
                        / (cdwt * cdc["DropChanceTotal"])
                    )
    ...
    for (cdt_id, cdc_id), target_ir_ids in cdt_cdc_ir_ids.items():
        ...
        for ir_id in target_ir_ids:
            ...
            if item_config.criterion == "number_of_kills_till_item":
                prob = 1.0 / item_config.number_of_kills_till_item
            elif item_config.criterion == "number_of_kills_till_item_per_mob":
                prob = min(
                    [
                        1.0
                        / item_config.number_of_kills_till_item_per_mob.get(
                            mob["MobID"], 1
                        )
                        for mobdrop in knowledge_base.cdtid_mobdrop_map[cdt_id]
                ...
                prob = min(
                    [
                        base_crate_prob
                        * get_crate_weight(cdc, crate_type)
                        / (cdwt * value)
        ...
        def search(lo: int, hi: int) -> int:
            ...
            current_values = [
                {ir_id: int(mi * value) for ir_id, value in values.items()}
                for values in float_values
            ]

            cond = all(
                abs(
                    sum(values.values()) / values[ir_id]
                    - base_crate_prob / orig_values[ir_id]
                )
                < 0.5
        ...
        for is_id, affected_ir_ids in itemset_ir_ids.items():
            if not any(ir_id in affected_ir_ids for ir_id in scaled_values):
                continue
